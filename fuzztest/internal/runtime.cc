...
FuzzTestFuzzerImpl::GetCorpusValueFromFile(absl::string_view path) {
...
void FuzzTestFuzzerImpl::ReplayInput(absl::string_view path) {
...
void FuzzTestFuzzerImpl::RunInUnitTestMode(const Configuration& configuration) {
  ...
  [&] {
    ...
    for (absl::string_view file :
         configuration.GetNonCrashingInputs(test_.full_name())) {
      ReplayInput(file);
    }
...
int FuzzTestFuzzerImpl::RunInFuzzingMode(int* /*argc*/, char*** /*argv*/,
  ...
  const int exit_code = [&] {
    ...
    PopulateFromSeeds(configuration.GetNonCrashingInputs(test_.full_name()));
