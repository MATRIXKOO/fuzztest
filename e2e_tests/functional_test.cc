...
std::string GetCorpusPath() {
  return absl::StrCat(
      getenv("TEST_SRCDIR"),
      "/com_google_fuzztest/e2e_tests/testdata/corpus");
}

TEST_F(UnitTestModeTest, UsesRegressionInputsOfDefaultCorpusByDefault) {
  auto [status, std_out, std_err] =
      Run("MySuite.DetectRegressionAndCoverageInputs", kDefaultTargetBinary,
          {{"FUZZTEST_FUZZ_FOR", "0s"}},  // No fuzzing needed
          {{"corpus_database", GetCorpusPath()}});
  EXPECT_THAT(status, Eq(ExitCode(0)));
  EXPECT_THAT(std_out, AllOf(HasSubstr("regression input detected"),
                             Not(HasSubstr("coverage input detected"))));
}

TEST_F(UnitTestModeTest,
       UsesRegressionAndCoverageInputsOfDefaultCorpusWhenReplayCorpus) {
  auto [status, std_out, std_err] =
      Run("MySuite.DetectRegressionAndCoverageInputs", kDefaultTargetBinary,
          {{"FUZZTEST_FUZZ_FOR", "0s"}},  // No fuzzing needed
          {{"corpus_database", GetCorpusPath()}, {"replay_corpus", "true"}});
  EXPECT_THAT(status, Eq(ExitCode(0)));
  EXPECT_THAT(std_out, AllOf(HasSubstr("regression input detected"),
                             HasSubstr("coverage input detected")));
}

TEST_F(UnitTestModeTest, UsesCrashingInputsOfDefaultCorpusWhenReproducing) {
  auto [status, std_out, std_err] = Run(
      "MySuite.CrashOnCrashingInput*", kDefaultTargetBinary,
      {{"FUZZTEST_FUZZ_FOR", "0s"}},  // No fuzzing needed
      {{"corpus_database", GetCorpusPath()}, {"reproduce_findings", "true"}});
  EXPECT_THAT(status, Eq(ExitCode(1)));
  EXPECT_THAT(
      std_out,
      AllOf(HasSubstr("[       OK ] MySuite.CrashOnCrashingInput"),
            HasSubstr(
                "[  FAILED  ] MySuite.CrashOnCrashingInput/replay/crashing1"),
            HasSubstr(
                "[  FAILED  ] MySuite.CrashOnCrashingInput/replay/crashing2")));
}

TEST_F(FuzzingModeCommandLineInterfaceTest, UsesCoverageInputsForSeedding) {
  auto [status, std_out, std_err] =
      RunWith({{"fuzz", "MySuite.DetectRegressionAndCoverageInputs"},
               {"fuzz_for", "1s"},
               {"corpus_database", GetCorpusPath()},
               {"replay_corpus", "true"}});
  EXPECT_THAT(std_out, AllOf(HasSubstr("regression input number 1"),
                             HasSubstr("coverage input number 1")));
  EXPECT_THAT(status, Eq(ExitCode(0)));
}
